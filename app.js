...
const state={page:"home",returnPage:"home",eventId:null,productId:null,scheduleId:null,orderId:null,calendarDate:new Date(),selectedDate:new Date(),filters:{events:{type:"",keyword:""},products:{type:"",keyword:""},schedules:{type:"",keyword:""}},scheduleSections:{current:true,future:true,past:false}};
...
function home(){
 ...
 title("ホーム");
 document.getElementById("screen").innerHTML=`<div class="hero"><h2>抽選管理</h2><div class="sub">イベントを中心に、公演日と申込・注文をまとめて管理</div></div><div class="grid"><div class="card stat" onclick="go('events')"><strong>${events.length}</strong><span>イベント</span></div><div class="card stat" onclick="go('products')"><strong>${products.length}</strong><span>商品</span></div><div class="card stat"><strong>${events.reduce((n,e)=>n+(e.applications||[]).length,0)}</strong><span>申込・注文</span></div><div class="card stat" onclick="go('schedules')"><strong>${schedules.length}</strong><span>予定</span></div></div><div class="section"><h2>最近のイベント</h2><span class="count">${recentEvents.length}件</span></div>${recentEvents.length?`<div class="list">${recentEvents.map(eventCard).join("")}</div>`:`<div class="empty">今後の公演予定があるイベントはありません。</div>`}`
}
...
function calendar(){
 ...
 function getItems(k){
   ...
   schedules.forEach(x=>{
     if((x.date||String(x.start||"").slice(0,10))===k) { const times=[x.meetingTime?`集合 ${x.meetingTime}`:"",x.startTime?`開始 ${x.startTime}`:""].filter(Boolean).join(" / "); addCalendarItem(items,{name:x.name,text:times||"予定",type:"予定",cls:"cal-schedule"}); }
   });
   return items;
 }
 let cells="";
